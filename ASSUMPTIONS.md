...
| A-04 | PostgreSQL is the system of record. Prisma 6.16.2 is the ORM. The first migration covers platform tables only against a non-production database. | A different ORM would be a new decision before domain migrations accumulate. |
| A-05 | Vercel hosts the web application. Railway remains the intended host for managed PostgreSQL and later workers. Until Railway is connected, local PostgreSQL and `docker-compose.yml` are the non-production database path. | The business can choose Neon or another Postgres host before production. |
...
| A-12 | Node.js is pinned to 24.21.0 and the package manager to pnpm 10.33.3, matching the existing Vercel Node.js 24.x setting. See D-013. | A later decision can move the pin and the Vercel setting together. |
